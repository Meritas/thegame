...
class Controller_Fights extends Controller_Template
{
	...
	public function action_next($id)
	{
		...
		if(Input::post()){
			...
			$fight_state->add('hp', 'Enemy HP', array('type'=>'hidden', 'value'=>(Input::post('hp') - rand( $result['dmg_min'],$result['dmg_max']) ) ));
			$fight_state->add('ch_hp', 'Character HP', array('type'=>'hidden', 'value'=>(Input::post('ch_hp') - rand(1, $result_ch['ch_stat_str']))));
